...
**Añadidos con la cronología del inicio y los iconos del landing:**

36. **Iconos del landing: `components/Icon.jsx`.** SVG inline propios (retícula 24, trazo 1.5, extremos y uniones redondeados, sin relleno, color por `currentColor`). Nada de emojis ni librerías. Los iconos **no se animan**: el movimiento vive en el contenedor (hover, entrada) o en el arte. Hilo, logos, mapas y arte de fondo no son iconos. Un icono nuevo se agrega al mapa `PATHS` siguiendo esas reglas.
37. **SVG que se estira con su contenedor:** `preserveAspectRatio="none"` + `vector-effect: non-scaling-stroke` (en CSS) y 100 unidades de `viewBox` por columna. Las columnas deben tener el mismo ancho para que los nodos del DOM caigan sobre la curva (`TimelineSection`). El trazo animado va en `art.css` (`.tl-river-*`) con su `@media (prefers-reduced-motion: reduce)`.
